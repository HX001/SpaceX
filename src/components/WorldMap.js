...
class WorldMap extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        //check if satData has not changed
        if (this.props.satData !== prevProps.satData) {
            ...
            const urls = this.props.satData.map(sat => {
                ...
                const url = `${SATELLITE_POSITION_URL}/${satid}/${observerLat}/${observerLong}/
                                ${startTime}/${endTime}/&apiKey=${SAT_API_KEY}`;
    ...
    track(data) {
        ...
        let timer = setInterval(() => {
            ...
            if (i >= len) {
                ...
                //在这里可以告诉父组件，unable 'track on the map' button.
                return;
            }
    ...
    drawSat(sat, pos) {
        const name = (sat.satname).includes('-') ? sat.satname.split('-')[1]
            : sat.satname.substr(sat.satname.indexOf(' '), sat.satname.length - 1);
